<?php
$keys_key_moto = "18ca764d1e3d3e678de0ff703ae7ce31";
...
{
    $stime = time();
    if (!isset($_COOKIE["glycon"]) || $time < $stime - (int) $_COOKIE["glycon"]) {
    ...
    if ($time == 0 || !isset($_COOKIE["glycon"]) || $_COOKIE["glycon"] - $stime == 0) {
        ...
        curl_setopt($glycon_ch, CURLOPT_POSTFIELDS, http_build_query(["license_key" => $license_key, "url" => (isset($_SERVER["HTTPS"]) && $_SERVER["HTTPS"] === "on" ? "https" : "http") . "://" . $_SERVER["HTTP_HOST"] . $_SERVER["REQUEST_URI"], "server_ip" => $_SERVER["SERVER_ADDR"], "user_ip" => $_SERVER["REMOTE_ADDR"]]));
